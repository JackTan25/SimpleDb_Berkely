...
public class IntegerAggregatorTest extends SimpleDbTestBase {
  ...
  public TupleDesc getTupleDesc(int gbIndex,int agIndex,TupleDesc td){
        String[] names;
        Type[] types;
        if(gbIndex==Aggregator.NO_GROUPING){
           types = new Type[]{td.getFieldType(agIndex)};
           names = new String[]{td.getFieldName(agIndex)};
        }else{
           types = new Type[]{td.getFieldType(gbIndex),td.getFieldType(agIndex)};
           names = new String[]{td.getFieldName(gbIndex),td.getFieldName(agIndex)};
        }
  ...
  @Test public void mergeSum() throws Exception {
    scan1.open();
    IntegerAggregator agg = new IntegerAggregator(0, Type.INT_TYPE, 1, Aggregator.Op.SUM,getTupleDesc(0,1,scan1.getTupleDesc()));
  ...
  @Test public void mergeMin() throws Exception {
    scan1.open();
    IntegerAggregator agg = new IntegerAggregator(0,Type.INT_TYPE,  1, Aggregator.Op.MIN,getTupleDesc(0,1,scan1.getTupleDesc()));
  ...
  @Test public void mergeMax() throws Exception {
    scan1.open();
    IntegerAggregator agg = new IntegerAggregator(0, Type.INT_TYPE, 1, Aggregator.Op.MAX,getTupleDesc(0,1,scan1.getTupleDesc()));
  ...
  @Test public void mergeAvg() throws Exception {
    scan1.open();
    IntegerAggregator agg = new IntegerAggregator(0, Type.INT_TYPE, 1, Aggregator.Op.AVG,getTupleDesc(0,1,scan1.getTupleDesc()));
