...
//                int id =
    ...
    public DbIterator physicalPlan(TransactionId t, Map<String,TableStats> baseTableStats, boolean explain) throws ParsingException {
        ...
        //调用Join优化器,statsMap和filterSelectivities对join连接进行优化获得新的joins
        //非0时才可以放入，防止返回null
        if(joins.size()!=0)
        ...
        while (joinIt.hasNext()) {
            ...
            DbIterator j;
            //从LogicalJoinNode获取连接的表名传入subPlanMap获取两张表的元组迭代器,然后调用Join优化器的instantiateJoin来对两种表进行join连接返回DbIterator
            j = jo.instantiateJoin(lj,plan1,plan2);
            //将新表放入subplanMap(key为左表名)
            subplanMap.put(t1name, j);
            //并移除右表，在equivMap里面放入t2name和t1name,然后将subplanMap里面的t2name移除
            if (!isSubqueryJoin) {
        ...
        //到这里应该已经吧join全部完成了,那么这里应该就应该存的只有最后合成完毕后的最后一个结果
        ...
        //接下来就是select语句后面选择的字段的内容了,循环每一个选择的指定的字段
