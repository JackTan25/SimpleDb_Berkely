...
public class BufferPool {
    ...
    public Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if(perm == Permissions.READ_ONLY){
            flag = lockManager.grantSLock(tid,pid);
        }else{
            flag = lockManager.grantXLock(tid,pid);
        }
        while(!flag){//没有申请到锁
            //判断是否发生了死锁
            if(lockManager.deadLock(tid,pid)){
    ...
    public boolean holdsLock(TransactionId tid, PageId p) {
        // some code goes here
        // not necessary for proj1
        List<PageId> allPagesByTid = lockManager.getAllPagesByTid(tid);
        for (PageId pageId : allPagesByTid) {
            if(pageId.equals(p)){
                return true;
            }
        }
     ...
     * @param tid
     */
    public synchronized void reverseTransaction(TransactionId tid) throws IOException {
        List<PageId> pids = lockManager.getAllPagesByTid(tid);
        for (PageId pid : pids) {
            if(lruPagesPool.get(pid).isDirty()!=null){
                lruPagesPool.reCache(pid);
            }
        }
    }
    /**
